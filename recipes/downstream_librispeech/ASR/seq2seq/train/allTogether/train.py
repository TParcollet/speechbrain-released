...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        dim = feats.size()[1]
        self.workers_regressors = {
            "age": self.modules.age,
            "gen": self.modules.gen,
            "melfs": self.modules.dec,
            "accent": self.modules.accent,
        ...
        if "age" in self.considered_workers:
            workers_predictions["age"] = torch.mean(
                workers_predictions["age"], dim=1
            )
        if "gen" in self.considered_workers:
            workers_predictions["gen"] = torch.mean(
                workers_predictions["gen"], dim=1
            )
        if "accent" in self.considered_workers:
            workers_predictions["accent"] = torch.mean(
                workers_predictions["accent"], dim=1
            )
        if "quality" in self.considered_workers:
            workers_predictions["quality"] = torch.squeeze(
                torch.mean(workers_predictions["quality"], dim=1)
            )
        for signal_worker in signal_workers:
            if signal_worker in self.considered_workers:
                if signal_worker not in [
                    "Loudness_sma3",
                    "mfcc",
                    "gammatone",
                    "melfs",
                    ...
                    workers_predictions[signal_worker] = torch.mean(
                        workers_predictions[signal_worker], dim=1
                    )
                else:

                    workers_predictions[signal_worker] = torch.squeeze(
                        workers_predictions[signal_worker]
                    )
    ...
    def compute_objectives(self, predictions, batch, other_targets, stage):
        # Load prediction
        self.workers_losses = {
            "age": self.hparams.classification_loss,
            "gen": self.hparams.classification_loss,
            "melfs": self.hparams.reconstruction_loss,
            "gammatone": self.hparams.reconstruction_loss,
            "accent": self.hparams.classification_loss,
        ...
        if len(exoworkers) > 0:
            ...
            for ind, worker in enumerate(exoworkers):
                if worker not in ["melfs", "mfcc", "gammatone"]:
                    worker_values = [
                        workers_targets_values[i][ind] for i in range(Nb)
                    ]
        ...
        for worker in self.considered_workers:
            self.workers_loss_recap[worker].append(
                (workers_loss[worker].item())
            )
            if len(self.workers_loss_recap[worker]) == 100:

                print(
                    f" for this worker : {worker} ; mean loss is {np.mean(self.workers_loss_recap[worker])}"
                )
                outfile.write(
                    f" for this worker : {worker} ; mean loss is {np.mean(self.workers_loss_recap[worker])}"
                )
