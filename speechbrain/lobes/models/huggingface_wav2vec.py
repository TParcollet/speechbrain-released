...
class HuggingFaceWav2Vec2ForPretrain(nn.Module):
    """This lobe enables the integration of HuggingFace
    pretrained wav2vec2.0 models.
    ...
    def forward(self, wav, mask_time_indices, stage):
        """Takes an input waveform and return its corresponding wav2vec encoding.
        ...
        if stage == sb.Stage.TRAIN:
            out = self.model(wav, mask_time_indices=mask_time_indices)
            loss = out.loss
            return loss
        else:
            out = self.model(wav, mask_time_indices=mask_time_indices)
...
class PretrainedHuggingFaceWav2Vec2(nn.Module):
    """This lobe enables the integration of HuggingFace
    pretrained wav2vec2.0 models.
